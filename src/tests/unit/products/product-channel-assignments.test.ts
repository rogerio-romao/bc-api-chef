...
import ProductChannelAssignments from '@/v3Api/Products/ProductChannelAssignments.ts';
...
describe('ProductChannelAssignments class', () => {
    let channelAssignments: ProductChannelAssignments;

    beforeEach(() => {
        mockTchef.mockReset();
        channelAssignments = new ProductChannelAssignments('test-token', BASE_URL);
    });

    describe('upsert', () => {
        beforeEach(() => {
            // BC returns 204 No Content for PUT channel-assignments
            mockTchef.mockResolvedValue({ data: '', ok: true });
        });

        it('makes exactly one HTTP call', async () => {
            await channelAssignments.upsert([mockAssignment]);
        ...
        it('targets the channel-assignments endpoint', async () => {
            await channelAssignments.upsert([mockAssignment]);

            expect(getCallUrl(mockTchef, 0).pathname).toMatch(/\/channel-assignments$/u);
        });
        ...
        it('returns null data on success (BC returns 204 No Content)', async () => {
            const result = await channelAssignments.upsert([mockAssignment]);
        ...
        it('accepts multiple assignments in a single call without error', async () => {
            const second = { channel_id: 20, product_id: 42 };

            const result = await channelAssignments.upsert([mockAssignment, second]);
    ...
    describe('getMultiple', () => {
        ...
        it('makes exactly one HTTP call when there is a single page', async () => {
            await channelAssignments.getMultiple();
        ...
        it('returns the collected data array', async () => {
            const result = await channelAssignments.getMultiple();
        ...
        it('appends product_id:in to the query string when provided', async () => {
            await channelAssignments.getMultiple({ 'product_id:in': 42 });

            expect(getCallUrl(mockTchef, 0).searchParams.get('product_id:in')).toBe('42');
        });

        it('appends channel_id:in to the query string when provided', async () => {
            await channelAssignments.getMultiple({ 'channel_id:in': 10 });

            expect(getCallUrl(mockTchef, 0).searchParams.get('channel_id:in')).toBe('10');
        });

        it('does not duplicate user-supplied page and limit in the query string', async () => {
            await channelAssignments.getMultiple({ limit: 25, page: 2 });
        ...
        it('fetches a single page when total_pages is 1', async () => {
            mockTchef.mockResolvedValue(
                makePageResponse([mockAssignment, { channel_id: 20, product_id: 42 }], 1, 1),
            );
        ...
        it('fetches all pages and concatenates results when total_pages > 1', async () => {
            mockTchef
                .mockResolvedValueOnce(makePageResponse([{ channel_id: 10, product_id: 42 }], 1, 3))
                .mockResolvedValueOnce(makePageResponse([{ channel_id: 20, product_id: 42 }], 2, 3))
                .mockResolvedValueOnce(
                    makePageResponse([{ channel_id: 30, product_id: 42 }], 3, 3),
                );
        ...
        it('fetches only the user-supplied page and stops', async () => {
            ...
            const result = await channelAssignments.getMultiple({ limit: 50, page: 2 });
        ...
        it(`clamps limit above ${PER_PAGE_MAX} down to ${PER_PAGE_MAX}`, async () => {
            await channelAssignments.getMultiple({ limit: PER_PAGE_MAX + 50 });
        ...
        it(`clamps limit below ${PER_PAGE_MIN} up to ${PER_PAGE_MIN}`, async () => {
            await channelAssignments.getMultiple({ limit: 0 });
        ...
        it('passes through a limit within the valid range unchanged', async () => {
            await channelAssignments.getMultiple({ limit: 100 });
        ...
        it('does not serialize retries into the query string', async () => {
            await channelAssignments.getMultiple({ retries: { repeat: 2, retryDelay: 0 } });
    ...
    describe('remove', () => {
        ...
        it('returns a 400 error without calling the API when no filter is provided', async () => {
            const result = await channelAssignments.remove({});
        ...
        it('makes exactly one HTTP call when product_id:in is provided', async () => {
            await channelAssignments.remove({ 'product_id:in': 42 });

            expect(mockTchef).toHaveBeenCalledOnce();
        });

        it('makes exactly one HTTP call when channel_id:in is provided', async () => {
            await channelAssignments.remove({ 'channel_id:in': 10 });
        ...
        it('appends channel_id:in to the query string', async () => {
            await channelAssignments.remove({ 'channel_id:in': 10 });
        ...
        it('appends both filters when both are provided', async () => {
            await channelAssignments.remove({ 'channel_id:in': 10, 'product_id:in': 42 });

            const url = getCallUrl(mockTchef, 0);

            expect(url.searchParams.get('product_id:in')).toBe('42');
            expect(url.searchParams.get('channel_id:in')).toBe('10');
        });

        it('returns null data on success', async () => {
            const result = await channelAssignments.remove({ 'product_id:in': 42 });
        ...
        it('forwards retries to the underlying HTTP call', async () => {
            await channelAssignments.remove({
                'product_id:in': 42,
