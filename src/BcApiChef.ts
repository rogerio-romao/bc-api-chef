...
export default class BcApiChef {
     ...
     * @param options.validate - When `true`, client-side payload validation runs on
     * mutating requests (e.g. `createProduct`, `updateProduct`) before they are sent
     * to BigCommerce, returning an error result on the first violation. Defaults to `false`.
     ...
     * @todo `options.validate` is not yet honoured — payload validation currently
     * runs unconditionally in `ProductsV3.createProduct` / `updateProduct`.
