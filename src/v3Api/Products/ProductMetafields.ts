...
export default class ProductMetafields {
    ...
    >(
        ...
        metafieldId: number,
        query?:
            | (Omit<ApiMetafieldQuery, 'include_fields' | 'exclude_fields'> & {
                  include_fields?: F;
        ...
        if (!Number.isInteger(metafieldId) || metafieldId <= 0) {
            return { error: 'Invalid metafieldId', ok: false, statusCode: 400 };
        }
